...
node* ADD(node* &root, node* &current, node* parent, node* grandparent, int data);
...
node* ADD(node* &root, node* &current, node* parent, node* grandparent, int data) {
  ...
  // same as binary search tree:
    ...
    current->setLeft(ADD(root, l,current, parent, data));
  }
  else {
    node* r = current->getRight();
    current->setRight(ADD(root, r, current, parent, data));
  }

  return current;

  // now check red black properties
